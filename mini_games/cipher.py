...
def shift(plaintext, key=3):
    ...
    for i in plaintext:
        original_word.append(ord(i))

    for i in original_word:
        if 97 <= i <= 122:
            if i + key <= 122:
                cypher_word.append(i + key)
            else:
                cypher_word.append(i + key - 26)
        elif 65 <= i <= 90:
            if i + key <= 90:
                cypher_word.append(i + key)
            else:
...
while turns < 3:
    for event in pygame.event.get():
        ...
        continue_text = font.render(s_original_word, True, (150, 150, 150))
        question_text = font.render((shift(original_word, int(guess))), True, (150, 150, 150))
        ...
        original_word = random_string()
        print(original_word)
        shift(original_word, 3)
        print(shift(original_word, 3))
        turns += 1
    pygame.display.update()